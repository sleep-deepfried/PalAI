...
import { TrendingUp, Activity, Award, BarChart3, Lightbulb } from 'lucide-react';
...
export function StatsContent({ initialScans }: StatsContentProps) {
  ...
  const diseaseCounts = scans.reduce(
    (acc, scan) => {
  ...
  const topDiseaseLabel = topDisease
  ...
  const labelCounts = scans.reduce(
    (acc, scan) => {
      acc[scan.label] = (acc[scan.label] || 0) + 1;
      return acc;
    },
  ...
  if (healthScore >= 50) {
    ...
    insights.push(
      `Only ${healthScore.toFixed(0)}% of leaves are healthy - consider preventive measures`
    );
  }
  ...
  if (recentScans.length >= 5 && olderScans.length >= 5) {
    const recentHealthy =
      recentScans.filter((s) => s.label === 'HEALTHY').length / recentScans.length;
  ...
  return (
    <div className="min-h-screen bg-ivory pb-20">
      ...
      <div className="bg-gradient-to-br from-olive-500 via-olive-600 to-olive-700 text-white px-4 pt-24 pb-8 safe-area-top relative overflow-hidden">
        {/* Background Pattern */}
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-sunbeam/30 p-2.5 rounded-xl backdrop-blur-sm">
          ...
          <p className="text-base sm:text-lg text-olive-100">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-8 pb-6 space-y-6">
        {totalScans === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-md">
            ...
            <p className="text-sm text-gray-600">Start scanning rice leaves to see insights</p>
          </div>
        ) : (
          <>
            {/* Hero Stats */}
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <StatCard icon={Activity} label="Total Scans" value={totalScans} color="blue" />
                <StatCard
                  icon={TrendingUp}
                  label="Health Score"
              ...
              <div className="max-w-md mx-auto">
                <StatCard icon={Award} label="Top Disease" value={topDiseaseLabel} color="orange" />
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-md">
                <h3 className="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2">
                  <Lightbulb className="w-5 h-5 text-sunbeam-500" />
                  Key Insights
                </h3>
                <p className="text-sm text-gray-600 mb-4">What the data is telling us</p>
                ...
                {insights.length > 0 ? (
                  <div className="space-y-3">
                    {insights.map((insight, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 p-3 bg-olive-50 rounded-lg border border-olive-100"
                      >
            ...
            {/* Trends Over Time */}
            {trendData.some((d) => d.scans > 0) && <TrendChart data={trendData} />}
